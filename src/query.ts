...
namespace gkeUsageMetering {
  ...
  export function generateSQLQuery(
    ...
    SELECT
      DATE(resource_usage.start_time) AS start_time,
      ...
      billing_table.rate,
      SUM(resource_usage.usage.amount) AS amount,
      NULL as amount_with_untracked
    FROM
      \`${fullUsageExportTableID}\` AS resource_usage, UNNEST(labels) as labels
    INNER JOIN
      billing_table
    ON
    ...
    WHERE
      ...
      AND _PARTITIONTIME <= TIMESTAMP("${endDate}")
    GROUP BY
      DATE(resource_usage.start_time),
      resource_usage.cluster_location,
      resource_usage.cluster_name,
      labels.key,
      labels.value,
      resource_usage.namespace,
      resource_usage.project.id,
      resource_usage.resource_name,
      resource_usage.sku_id,
      billing_table.rate
    ),
    ...
    SELECT
      ...
      DATE(billing_table.min_usage_start_time) AS start_time,
    ...
    filtered_resource_usage_with_unused AS (
      SELECT
        ...
        filtered_resource_usage.key,
        filtered_resource_usage.value,
    ...
    filtered_resource_usage_with_unused_and_unallocated AS (
      SELECT
        ...
        filtered_resource_usage_with_unused.key,
        filtered_resource_usage_with_unused.value,
    ...
    SELECT
      resource_usage.cluster_location,
      resource_usage.cluster_name,
      ARRAY_AGG(STRUCT(resource_usage.key, resource_usage.value)) as labels,
      resource_usage.namespace,
      resource_usage.resource_name,
      STRING(NULL) AS resource_name_with_type_and_unit,
      resource_usage.sku_id,
      FORMAT_DATE('%Y%m%d', resource_usage.start_time) AS usage_start_time,
      0 AS amount,
...
SELECT
  ...
  resource_usage.cluster_location,
  resource_usage.cluster_name,
  CAST(labels.key AS STRING) AS key,
  CAST(labels.value AS STRING) AS value,
  resource_usage.namespace,
  resource_usage.project.id AS project_id,
  resource_usage.resource_name,
  resource_usage.sku_id,
...
consumption_based_cost_allocation AS (
    WITH
      ...
      SELECT
        ...
        SUM(resource_usage.usage.amount) AS amount,
      FROM
        \`${fullConsumptionUsageExportTableID}\` AS resource_usage, UNNEST(labels) as labels
      INNER JOIN
        billing_table
      ON
        resource_usage.sku_id = billing_table.sku_id
      ...
      SELECT
        filtered_resource_usage.cluster_location,
        filtered_resource_usage.cluster_name,
        ARRAY_AGG(STRUCT(filtered_resource_usage.key, filtered_resource_usage.value)) as labels,            filtered_resource_usage.namespace,
        filtered_resource_usage.resource_name,
        CASE
        ...
        AS resource_name_with_type_and_unit,
        filtered_resource_usage.sku_id,
        FORMAT_date('%Y%m%d', filtered_resource_usage.start_time) AS usage_start_time,
        CASE
          WHEN resource_name = 'cpu' THEN amount/3600
          WHEN resource_name = 'memory' THEN amount/(3600*POW(2,30))
          END
      ...
      GROUP BY
        ...
        filtered_resource_usage.sku_id,
        filtered_resource_usage.amount,
        filtered_resource_usage.amount * filtered_resource_usage.rate
  )
SELECT
*
FROM
